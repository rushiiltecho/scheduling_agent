...
from agent_service.shared_libraries.atlassian_api_toolset_new import ConfluenceApiToolset as JiraApiToolset
...
CONFLUENCE_ACCESS_TOKEN = os.getenv("CONFLUENCE_ACCESS_TOKEN") or "DEFAULT"
...
jira_tool_set = JiraApiToolset(
    ...
    tool_filter=confluence_tool_filter#jira_knowledge_extraction_functions#confluence_tool_filter#["search_content_by_cql","get_current_user","get_audit_records"]
)
jira_tool_set.configure_access_token_auth(CONFLUENCE_ACCESS_TOKEN)
...
def jira_before_agent_callback(callback_context: CallbackContext):
    ...
    print(f"\n{'*'*60}\n{'*'*60}\n\nCONFLUENCE CALLBACK_CONTEXT [STATE] (Before agent):\n{callback_context._invocation_context.session.state}\n\n{'*'*60}\n{'*'*60}\n")
    if callback_context._invocation_context.session.state:
        ...
        for key, value in state.items():  
            if 'temp:jira' in key:
                ...
                print("REINITIALIZED CONFLUENCE TOOLS 2")
...
root_agent = Agent(
    ...
    instruction=CONFLUENCE_INSTRUCTIONS,#"You are a helpful Jira and Drive Agent, where you have access to Jira and Drive agents, which you call accordingly wherever necessary." + f"\n{INSTRUCTIONS}\n\n{GOOGLE_INSTRUCTION}\n\n"+"Always Modify the final response as Markdown Tables wherever possible and applicable, without the Markdown triple quotes",
    name="jira_agent",
