...
# async def invoke_calendar_agent(tool_context:ToolContext):
#   artifact_service=tool_context._invocation_context.artifact_service or InMemoryArtifactService(),
#   session_service=tool_context._invocation_context.session_service or None,
#   runner = Runner(
#     app_name="scheduling_agent",
#     agent=calendar_agent,
#     artifact_service=artifact_service or InMemoryArtifactService(),
#     session_service=session_service or None,
#     )
...
async def invoke_calendar_agent(tool_context: ToolContext):
    """
    Tool to call the calendar agent, automatically handling OAuth
    by inheriting services from the invocation context.
    """
    # 1. Grab the user’s question (or default)
    query = tool_context.input or "Show me my user info"

    # 2. Wrap your calendar_agent as a tool—no services passed here!
    calendar_tool = AgentTool(agent=calendar_agent)

    # 3. Run it; the ADK runtime pulls in session/artifact services & does OAuth as needed.
    calendar_response = await calendar_tool.run_async(
        args={"request": query},
        tool_context=tool_context
    )

    # 4. Store and return
    tool_context.state["calendar_response"] = calendar_response
